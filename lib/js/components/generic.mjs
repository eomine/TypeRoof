...
export function collapsibleMixin(element, buttonSelector, isInitiallyOpen=false) {
    ...
    const onClickToggler = (e) => {
        e.preventDefault();
        const wasOpen = element.classList.contains(openClassName);
        const isOpen = !wasOpen;
        e.currentTarget.setAttribute('aria-expanded', isOpen);
        if (isOpen) {
            element.classList.add(openClassName);
            element.classList.remove(closedClassName);
            element.scrollIntoView({block: 'start', inline: 'nearest', behavior: 'smooth'});
        } else {
            element.classList.remove(openClassName);
            element.classList.add(closedClassName);
        }
    };

    const getToggler = () => element.querySelector(buttonSelector);

    const onMutation = (_, observer) => {
        const toggler = getToggler();
        if (!toggler) return;

        toggler.classList.add('ui_collapsible_mixin_toggler');
        toggler.setAttribute('aria-role', 'button');
        toggler.addEventListener('click', onClickToggler);
        observer?.disconnect();
    }
...
export class Collapsible extends _BaseContainerComponent {
    ...
    constructor(widgetBus, togglerLabel, _widgets) {
        const element = widgetBus.domTool.createElement('div', {class: Collapsible.BASE_CLASS});
        const togglerElement = widgetBus.domTool.createElement('div', {class: `${Collapsible.BASE_CLASS}-toggler`, role: 'button', 'aria-expanded': false});
        const contentsElement = widgetBus.domTool.createElement('div', {class: `${Collapsible.BASE_CLASS}-contents`, 'aria-hidden': true});
        const zones = new Map([['local', contentsElement]]);
